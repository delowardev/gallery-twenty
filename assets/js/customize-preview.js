/* global galleryTwentyBgColors, galleryTwentyPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( galleryTwentyBgColors ).forEach( function( context ) {
					galleryTwentyGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function galleryTwentyGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'gallery_twenty-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#gallery_twenty-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( galleryTwentyPreviewEls[ context ] ) {
				_.each( galleryTwentyPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		galleryTwentyGenerateColorA11yPreviewStyles( 'content' );
		galleryTwentyGenerateColorA11yPreviewStyles( 'header-footer' );
	} );

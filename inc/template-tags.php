 ...
 */
function gallery_twenty_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `gallery_twenty_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters( 'gallery_twenty_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'gallery_twenty_site_logo', $html, $args, $classname, $contents );
 ...
 */
function gallery_twenty_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'gallery_twenty_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function gallery_twenty_is_comment_by_post_author( $comment = null ) {
 ...
 */
function gallery_twenty_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'gallery_twenty_filter_comment_reply_link' );
 ...
 */
function gallery_twenty_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo gallery_twenty_get_post_meta( $post_id, $location ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in gallery_twenty_get_post_meta().
 ...
 */
function gallery_twenty_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'gallery-twenty' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . gallery_twenty_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'gallery_twenty_edit_post_link', 10, 3 );
 ...
 */
function gallery_twenty_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'gallery_twenty_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		*/
		$post_meta = apply_filters(
			'gallery_twenty_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		...
		*/
		$post_meta = apply_filters(
			'gallery_twenty_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 *
				 * @since 1.0.0
				 * @since Gallery Twenty 1.1 Added the `$post_meta` and `$location` parameters.
				 *
				 * @param int    $post_id   Post ID.
				 ...
				 */
				do_action( 'gallery_twenty_start_of_post_meta_list', $post_id, $post_meta, $location );
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php gallery_twenty_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'gallery_twenty_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function gallery_twenty_filter_wp_list_pages_item_classes( $css_class, $item, $depth, $args, $current_page ) {
 ...
 */
function gallery_twenty_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
...
add_filter( 'nav_menu_item_args', 'gallery_twenty_add_sub_toggles_to_main_menu', 10, 3 );
 ...
 */
function gallery_twenty_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = GalleryTwenty_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = gallery_twenty_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'gallery_twenty_nav_menu_social_icons', 10, 4 );
 ...
 */
function gallery_twenty_no_js_class() {
...
add_action( 'wp_head', 'gallery_twenty_no_js_class' );
 ...
 */
function gallery_twenty_body_classes( $classes ) {
...
add_filter( 'body_class', 'gallery_twenty_body_classes' );
 ...
 */
function gallery_twenty_get_the_archive_title( $title ) {

	$regex = apply_filters(
		'gallery_twenty_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'gallery_twenty_get_the_archive_title' );
 ...
 */
function gallery_twenty_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'gallery_twenty_toggle_duration', 250 );
 ...
 */
function gallery_twenty_unique_id( $prefix = '' ) {
	static $id_counter = 0;
